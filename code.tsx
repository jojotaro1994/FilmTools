...
const EmotionSettingsModal = ({ isOpen, onClose, emotions, onAddEmotion, onDeleteEmotion }) => { const [newEmotionName, setNewEmotionName] = useState(''); if (!isOpen) return null; const handleAdd = () => { if (newEmotionName.trim()) { onAddEmotion(newEmotionName.trim()); setNewEmotionName(''); } }; return ( <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4" onClick={onClose}> <div className="bg-gray-800 rounded-lg shadow-xl p-6 relative w-full max-w-md" onClick={(e) => e.stopPropagation()}> <div className="flex items-start mb-4"><Settings size={24} className="text-amber-400 mr-3 mt-1"/><div><h2 className="text-xl font-semibold text-white">Emotions to Analyze</h2><p className="text-gray-400 mt-1">Select and customize the emotions you want to track.</p></div><button onClick={onClose} className="absolute top-4 right-4 text-gray-400 hover:text-white" aria-label="Close"><X size={24} /></button></div> <div className="my-6"><div className="flex flex-wrap gap-2">{emotions.map(emotion => (<div key={emotion.key} className="flex items-center bg-gray-700 rounded-full pl-3 pr-1 py-1 text-sm"><span className="w-2 h-2 rounded-full mr-2" style={{ backgroundColor: emotion.color }}></span><span className="text-gray-200">{emotion.label}</span><button onClick={() => onDeleteEmotion(emotion.key)} className="ml-2 text-gray-500 hover:text-white"><X size={16}/></button></div>))}</div></div> <div className="flex items-center space-x-2"><input type="text" value={newEmotionName} onChange={(e) => setNewEmotionName(e.target.value)} placeholder="Add new emotion (e.g. Joy, Fear)" className="flex-grow bg-gray-700 border border-gray-600 rounded-md px-3 py-2 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-amber-500"/><button onClick={handleAdd} className="bg-amber-500 hover:bg-amber-600 text-white font-bold py-2 px-4 rounded-md flex items-center space-x-2"><Plus size={16}/><span>Add</span></button></div> </div> </div> ) };
...
export default function App() {
    ...
    const handleFileChange = (event) => {
        ...
        setAppData(prev => {
            ...
            const updatedData = prev[dataKey].map(row => row.id === id ? { ...row, image: imageUrl } : row);
            return { ...prev, [dataKey]: updatedData };
        });
        event.target.value = null; setActiveUpload({ id: null, type: null });
    };
    ...
    const handleExport = () => {
        const dataToExport = {
            version: 2,
            ...appData,
            scriptData: appData.scriptData.map(({ image, ...rest }) => rest),
            storyboardData: appData.storyboardData.map(({ image, ...rest }) => rest),
        };
    ...
    const handleImportFile = (event) => {
        ...
        reader.onload = (e) => {
            try {
                ...
                if (importedData.version === 2 && importedData.scriptData && importedData.storyboardData && importedData.emotionColumns) {
                    const restoredScriptData = importedData.scriptData.map(row => ({ ...row, image: null }));
                    const restoredStoryboardData = importedData.storyboardData.map(panel => ({ ...panel, image: null }));
                    setAppData({
                        emotionColumns: importedData.emotionColumns,
                        scriptData: restoredScriptData,
                        storyboardData: restoredStoryboardData,
                    });
                } else { throw new Error("Invalid or outdated file format."); }
            } catch (error) {
    ...
    return (
        <div className="bg-gray-900 text-gray-200 min-h-screen font-sans">
                            ...
                            <div className="bg-gray-800 border border-gray-700 rounded-lg shadow-lg overflow-hidden">
                                ...
                                <div className="p-6 border-t border-gray-700 flex items-center justify-between flex-wrap gap-4">
                                    <div className="flex items-center space-x-6 text-sm">
                                        <button onClick={() => setIsEmotionSettingsOpen(true)} className="flex items-center space-x-2 text-gray-300 hover:text-white"><Settings size={16} /><span>Emotion Settings</span></button>
            ...
            <EmotionSettingsModal isOpen={isEmotionSettingsOpen} onClose={() => setIsEmotionSettingsOpen(false)} emotions={appData.emotionColumns} onAddEmotion={handleAddEmotion} onDeleteEmotion={handleDeleteEmotion} />
        </div>
    );
}
